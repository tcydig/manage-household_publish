...
struct EditExpensesView: View {
    ...
    // model
    @ObservedObject var viewModel = EditExpensesViewModel()
    
    // pullDown of Category of expensse (default is Rent)
    ...
    @State private var indexOfExpenseSelected = 1
    // current status
    @State var currentStatus: String = "A"
    // modal status
    @State private var showingModal = false
    // scrolling menu of month
//    @State var viewModel.currentDay: Date = Date()
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 20){
                ...
                // input expense
                VStack(spacing: 10){
                    let  totalbalance = currentStatus == "A" ? viewModel.totalBalance + viewModel.inputExpense : viewModel.totalBalance - viewModel.inputExpense
                    ...
                    GridRow{
                        Menu {
                            ForEach(Array(categoryList.enumerated()), id: \.offset) { offset, item in
                                Button {
                                    indexOfExpenseSelected = offset + 1
                    ...
                    GridRow{
                        if (viewModel.inputExpense > 0 && viewModel.isEditable && viewModel.inputExpense > 0) {
                            Button(action: {
                                ...
                                ModalView(balanceEntered: viewModel.inputExpense,expenseName: selectedCategory,expenseId:indexOfExpenseSelected,status:currentStatus,userId:headerViewModel.userId,viewModel:viewModel,yearMonth:viewModel.currentDay)
                            }
                        } else {
                            Text("SEND")
                                .font(.title)
...
struct ModalView: View {
    ...
    var status: String
    var userId: Int
    var viewModel: EditExpensesViewModel
    var yearMonth: Date
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 20){
                ...
                Group{
                    ...
                    HStack{
                        Button(action: {
                            viewModel.pushExpense(expenseId:expenseId,balanceEntered:balanceEntered,status:status,userId: userId,yearMonth:yearMonth)
                            dismiss()
                        }) {
                            Text("Uploade")
                                .font(.system(size: 40))
